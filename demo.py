...
def run_automation():
    with sync_playwright() as p:
        ...
        try:
            ...
            page.goto("https://dev-hire-admin.bling-ai.com/portal/index.html",
            ...
            page.wait_for_timeout(5000)
            logging.info(
                "13. Calendar and Compare tab closed. New date range applied successfully.")
            ...
            # ---------------------------------------------------------
            # ADDED STEPS: 16-COMBINATION SANKEY MATRIX TESTING
            # ---------------------------------------------------------
            current_stage = "Testing All 16 Hire Signal Combinations"
            logging.info("18. Running the full 16-combination matrix test...")

            # Helper function to reliably click nodes.
            def click_signal_node(node_name, index=0):
                node = dashboard_frame.get_by_text(
            ...
            # Precise scroll to keep the diagram and all bottom cards fully in view
            hire_title = dashboard_frame.get_by_text(
                "Hire Signal Overview", exact=True).first
            hire_title.evaluate(
            ...
            # Map nodes to their respective indices so 'Hire' and 'Pending' don't click the wrong column
            nova_signals = [("Hire", 0), ("Consider", 0),
                            ("Do Not Hire", 0), ("Pending", 0)]
            recruiter_decisions = [
                ("Hire", 1), ("Decline", 0), ("Hold", 0), ("Pending", 1)]

            reset_btn = dashboard_frame.get_by_text(
                "Reset", exact=True).filter(visible=True).first

            # --- THE 16-COMBINATION LOOP ---
            for n_node, n_idx in nova_signals:
                for r_node, r_idx in recruiter_decisions:
                    logging.info(
                        f"Testing Flow: {n_node} (Nova) -> {r_node} (Recruiter)")

                    # 1. Reset state before every combination
                    if reset_btn.count() > 0:
                        reset_btn.click(force=True)
                        page.wait_for_timeout(400)

                    # 2. Click the combination
                    click_signal_node(n_node, n_idx)
                    page.wait_for_timeout(300)
                    click_signal_node(r_node, r_idx)

                    logging.info(
                        f"Waiting 3 seconds for UI to settle for {n_node} -> {r_node}...")
                    page.wait_for_timeout(3000)

                    # 3. Dynamic Cross-Validations
                    try:
                        # A. Validate Nova Bottom Card Title updated correctly
                        nova_title_regex = re.compile(
                            f"Nova Signal\\s*→\\s*{re.escape(n_node)}", re.I)
                        nova_bottom_card = dashboard_frame.locator(
                            "div").filter(has_text=nova_title_regex).first
                        expect(nova_bottom_card).to_be_visible(timeout=5000)

                        # B. Validate Recruiter Bottom Card Title updated correctly
                        rec_title_regex = re.compile(
                            f"Recruiter Decision\\s*→\\s*{re.escape(r_node)}", re.I)
                        rec_bottom_card = dashboard_frame.locator(
                            "div").filter(has_text=rec_title_regex).first
                        expect(rec_bottom_card).to_be_visible(timeout=5000)

                        # C. Extract the dynamic 'Decision Match' Count safely
                        decision_match_card = dashboard_frame.locator("div").filter(
                            has_text=re.compile(r"Decision Match", re.I)).first
                        dm_text = decision_match_card.text_content(
                            timeout=5000)
                        # Ensure dm_text is a string to satisfy type expectations
                        if dm_text is None:
                            dm_text = ""

                        # Use regex to grab the exact digits following "Decision Match"
                        match_search = re.search(
                            r'Decision Match\s*(\d+)', dm_text, re.I)
                        dm_value = match_search.group(
                            1) if match_search else "0"

                        # D. Validate 'Routed To' Box matches both the title and the dynamic count
                        routed_title_regex = re.compile(
                            f"ROUTED TO\\s*{re.escape(n_node)}\\s*/\\s*{re.escape(r_node)}", re.I)
                        routed_to_card = dashboard_frame.locator(
                            "div").filter(has_text=routed_title_regex).first
            ...
            page.wait_for_timeout(3000)
            logging.info(
                "24. Successfully reached 'Source By Candidate Applied'.")
